...
const CalendarPage = () => {
    ...
    const bookingDates = useMemo(() => {
        ...
        (events || []).forEach(event => {
            ...
            for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
                set.add(new Date(d).toDateString());
            }
        });
        return set;
    }, [events]);
    ...
    const bookingEventMap = useMemo(() => {
        ...
        (events || []).forEach(event => {
            ...
            const end = new Date(event.bookingEndDate);
            for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
                map.set(new Date(d).toDateString(), event);
            }
        });
        return map;
    }, [events]);
            ...
            if (bookingEventMap.has(key)) {
                ...
                return (
                    <div
                        ...
                        data-tooltip-content={`${t.events.bookingOpenFor} ${event.name} (${bs} — ${be})`}
                        className="event-dot"
                    ></div>
                );
            }
    ...
    return (
        ...
        >
            ...
            <h1 className="text-3xl md:text-4xl font-bold font-heading text-primaryDark text-center mb-8 border-b-2 border-primary pb-3">
                <FaCalendarAlt className="inline mr-3 text-primary" /> {t.events.calendar}
            </h1>
            <div className="p-4 bg-card rounded-2xl shadow-soft max-w-3xl mx-auto">
                ...
                <div className="flex justify-center space-x-6 mt-6 text-sm font-medium text-gray-700">
                    ...
                    <div className="flex items-center space-x-2">
                        <FaCircle style={{ color: theme.background }} /> <span>Today</span>
                    </div>
                </div>
            </div>
        </motion.div>
