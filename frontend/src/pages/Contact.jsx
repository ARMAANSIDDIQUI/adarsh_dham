...
// --- SVG Icons (Used directly in JSX) ---
...
const Contact = () => {
    ...
    const sendEmail = (e) => {
        ...
        if (serviceID.includes('YOUR_')) {
            toast.error('Email service is not configured. Please add your EmailJS keys in the emailJsConfig object.');
            setLoading(false);
            return;
        }
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="container mx-auto p-4 md:p-10 min-h-screen bg-neutral font-body">
            ...
            <div className="max-w-3xl mx-auto bg-card p-6 md:p-10 rounded-2xl shadow-soft text-gray-700 space-y-8">
                ...
                {emailJsConfig.serviceID.includes('YOUR_') && (
                    <div className="p-4 bg-highlight/10 border border-highlight text-highlight rounded-lg text-sm font-medium">
                        <p className="font-bold">Configuration Required:</p>
                        <p>Please update the `emailJsConfig` object at the top of this file with your actual EmailJS credentials to enable sending messages.</p>
                    </div>
                )}
