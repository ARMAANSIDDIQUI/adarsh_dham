    ...
    <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-background">
            <thead className="bg-background/50">
                <tr>
                    <th className="px-4 py-3 text-left text-xs font-medium font-heading text-primaryDark uppercase tracking-wider">Event Name</th>
                    <th className="px-4 py-3 text-left text-xs font-medium font-heading text-primaryDark uppercase tracking-wider">Location</th>
                    <th className="px-4 py-3 text-left text-xs font-medium font-heading text-primaryDark uppercase tracking-wider">Dates</th>
                    <th className="px-4 py-3 text-left text-xs font-medium font-heading text-primaryDark uppercase tracking-wider">Actions</th>
                </tr>
            </thead>
...
const ManageEvents = () => {
    ...
    const validateDates = (eventData) => {
        ...
        if (bs > be) return 'Booking start date cannot be after booking end date.';
        if (s > e) return 'Event start date cannot be after event end date.';
        if (bs > s) return 'Booking should start before the event starts.';
        if (be > e) return 'Booking should end before or when the event ends.';
        return null;
    };
        ...
        try {
            ...
            if (err.response?.status === 409) {
                setModalData({
                    ...
                    message: 'A booking or building is currently assigned to this event. Remove all dependencies before deleting.',
                    confirmText: 'Got It',
                    isAlert: true,
    ...
    const handleDeleteEvent = async (id) => {
        ...
        let message = `Are you sure you want to permanently delete the event "${eventName}"?`;

        try {
            const res = await api.get(`/events/${id}/bookings/exists`);
            if (res.data.exists) {
                message = `There are bookings for this event. If you delete this event, all associated bookings will also be deleted! Are you sure you want to permanently delete the event "${eventName}"?`;
            }
        } catch (error) {
            // If the check fails, proceed with the default message but log the error
            console.error("Could not check for bookings, proceeding with default delete confirmation.", error);
        }
