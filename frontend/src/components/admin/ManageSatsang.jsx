...
import { FaEdit, FaTrashAlt, FaPlus, FaSpinner, FaLink } from 'react-icons/fa';
...
const ManageSatsang = () => {
  ...
  const [newLink, setNewLink] = useState({ name: '', url: '', eventId: '' });
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 md:p-8 bg-gray-100 min-h-screen">
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg mb-8">
        ...
        <form onSubmit={handleAddLink} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            {/* Associated Event */}
            <div className="md:col-span-1">
              <label className="block text-sm font-medium text-gray-700">Associated Event</label>
              <select value={newLink.eventId} onChange={(e) => setNewLink({ ...newLink, eventId: e.target.value })} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-pink-300 focus:border-pink-500" required>
                <option value="">Select an Event</option>
                {(events || []).filter(e => e.name?.toLowerCase().includes('satsang')).map(event => (
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {(liveLinks || []).map(link => (
              <tr key={link._id} className="hover:bg-pink-50 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{(events || []).find(e => e._id === link.eventId)?.name || 'N/A'}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                  <div className="flex space-x-3">
                    <button onClick={() => setEditingLink(link)} className="text-pink-500 hover:text-pink-700 transition-colors" title="Edit Link">
            ...
            {liveLinks.length === 0 && (
                <tr className="border-b-0">
                    <td colSpan="4" className="px-6 py-8 text-center text-gray-500">No live links have been set up yet.</td>
                </tr>
            )}
          </tbody>
        </table>
      ...
      {editingLink && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full flex items-center justify-center z-[1000]">
          <div className="relative p-8 bg-white w-full max-w-md rounded-xl shadow-2xl m-4">
            <h3 className="text-2xl font-bold text-gray-800 mb-4">Edit Live Link: {editingLink.name}</h3>
            <form onSubmit={handleUpdateLink} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Associated Event</label>
                <select name="eventId" value={editingLink.eventId} onChange={(e) => setEditingLink({ ...editingLink, eventId: e.target.value })} className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-pink-300 focus:border-pink-500" required>
                  <option value="">Select an Event</option>
                  {(events || []).filter(e => e.name?.toLowerCase().includes('satsang')).map(event => (
                    <option key={event._id} value={event._id}>{event.name}</option>
                  ))}
