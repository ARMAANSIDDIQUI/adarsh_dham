    ...
    formData.people
      ...
      .map(({ p, i }, idx) => (
        <div key={i} className="grid grid-cols-2 gap-4 pt-4 border-b-2 border-background pb-4 last:border-b-0">
          <h4 className="col-span-2 font-bold capitalize text-primaryDark">{gender} #{idx + 1}</h4>
          <div>
            <label className="block text-xs font-medium text-gray-600">Name</label>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-[1000] p-4 font-body">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-card rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-soft">
        ...
        <div className="overflow-y-auto p-6 scrollbar-hide">
          <h2 className="text-3xl font-bold font-heading mb-8 text-center text-primaryDark border-b-2 border-background pb-3">Edit Your Booking</h2>
          <form onSubmit={handleUpdateSubmit} className="space-y-8">
            <div>
              <h3 className="text-xl font-semibold font-heading text-primaryDark mb-4 border-b border-background pb-2">Period of Stay</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <ThemedInput label="From" name="stayFrom" value={formData.stayFrom} onChange={handleChange} required type="date" icon={<FaCalendarAlt />} min={minStay} max={maxStay} />
                <ThemedInput label="To" name="stayTo" value={formData.stayTo} onChange={handleChange} required type="date" icon={<FaCalendarAlt />} min={formData.stayFrom || minStay} max={maxStay} />
              </div>
            </div>
