...
const BookingCard = ({ booking, onEdit, onDelete, onDownloadPdf, navigateToEvent }) => {
    ...
    const getStatusText = (status) => {
        switch (status) {
            case 'approved': return t.myBookings.status.approved;
            case 'pending': return t.myBookings.status.pending;
            case 'declined': return t.myBookings.status.declined;
        ...
        >
            <div className="flex justify-between items-start mb-4">
                <div>
                    ...
                    >
                        {booking.eventId?.name || t.myBookings.card.details}
                    </h3>
            ...
            {booking.status === 'declined' && <p className="mt-4 text-sm font-medium text-red-700">{t.myBookings.card.declinedMsg}</p>}
            {booking.status === 'pending' && <p className="mt-4 text-sm font-medium text-yellow-700">{t.myBookings.card.pendingMsg}</p>}
    ...
    if (bookings.length === 0) {
        return (
            <div className="flex flex-col justify-center items-center h-screen text-center px-4 bg-neutral font-body">
                ...
                <h2 className="text-2xl font-bold font-heading text-primaryDark mb-2">{t.myBookings.noBookings.title}</h2>
                <p className="text-gray-700">{t.myBookings.noBookings.desc}</p>
                <Link to="/calendar">
                    <Button className="mt-6 bg-primary hover:bg-primaryDark text-white">{t.myBookings.noBookings.button}</Button>
                </Link>
            </div>
        );
    }
    ...
    return (
        <>
            <div className="p-4 md:p-8 max-w-5xl mx-auto min-h-screen bg-neutral font-body">
                <h2 className="text-3xl font-bold font-heading mb-6 text-primaryDark border-b-2 border-primary pb-2 text-center">{t.myBookings.title}</h2>
