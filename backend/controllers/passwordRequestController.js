...
exports.createRequest = async (req, res) => {
    ...
    try {
        ...
        // Notify Admins
        const targetRoles = ['admin', 'super-admin'];
        const admins = await User.find({ roles: { $in: targetRoles } });
        const adminIds = admins.map(a => a._id.toString());

        if (adminIds.length > 0) {
            await createAndSaveNotification({
                message: `New password reset request from ${user.name} (${phone}). Reason: ${reason}`,
                userIds: adminIds,
                notifyAdmins: false // We manually selected them
            });
        }
